...
**It refuses to lie about a sale.** `NewPrice` must actually be lower than
`OldPrice` or the run fails loudly rather than announcing a price *rise*. Prices
can be written any way your sheet already writes them (`129`, `129.00`,
`$1,299.99`): the builder reads the number for the maths and keeps your
formatting on screen, and a bare number gets your `currencySymbol` back rather
than appearing as a naked `129` next to a `$40` savings chip.

**Every headline and pill is measured before it is drawn.** A 24-character brand
name, a 62-character product title and a 26-character CTA all step down a size —
and truncate with an ellipsis as a last resort — instead of spilling past the
canvas. Verified frame by frame on the production renderer at both extremes.
...
| **Build project JSON** | The whole design, and **two items out**. Price parsing and formatting, the percent-off and savings maths, the measured type ramps, the promo-code branch that grows the CTA card, HTML-escaping of all sheet/Shopify text, and the API's `name` character rules. |
...
- **Rendered on the production engine** (the same `@zvid-io/zvid` package the
  render farm runs) from the builder's real output — **six renders, three video
  and three banner**: the default fixture (30% off, `$249`→`$174`, a 15-character
  deadline → 13.4 s video + 1200×628 banner); a stress fixture (a 62-character
  product name, `$1,099.00`→`$551.00` so the 49.86% drop rounds to 50%, a
  24-character brand name, a 26-character CTA and a promo code, which grows the
  CTA card); and a minimal fixture (bare numeric prices with no symbol, **no**
  `EndsAt`, **no** promo code, **no** music and a 1080×1080 banner, which
  exercises the banner's rescale path).
- **Every extracted frame was reviewed** — 2 fps across all three videos plus
  exact-timestamp grabs at both transition midpoints and each final frame, **93
  frames in total**, alongside all three rendered PNGs. No clipping, no overflow,
  no text touching an edge, no low-contrast text, no broken animation states, and
  no unsubstituted variables. Two defects were found and fixed this way: the
  banner's CTA arrow could touch a short CTA label, and the banner's frame and
  wedge letterboxed instead of filling a non-1200×628 canvas.
- **Remote validation against the live API** (`POST
  /api/render/validate/api-key` via MCP with `remote: true`) on **both distinct
  payload shapes**. The video payload: `valid: true`, **0 errors, 0 warnings**,
  `creditsRequired: 14`, resolved duration 13.4 s, schema **1.0.0**. The
  `type: "image"` banner payload: `valid: true`, **0 errors, 0 warnings**,
  `creditsRequired: 1`, schema **1.0.0**.
- **Every pinned URL HEAD-checked at authoring time**: the default music bed
  (HTTP 200, `audio/mpeg`, 3,695,616 bytes — under the 5 MB plan cap), the stress
  fixture's bed (HTTP 200, `audio/mpeg`, 3,722,344 bytes) and both fixture
  product images (HTTP 200, `image/jpeg`).
- **The embedded code node is byte-identical** to the frame-reviewed standalone
  ...
  used on every key.
- **48 simulated-execution checks** run the shipped workflow's own code nodes
  ...
  *rise* as a sale, and turns 401 / unreachable / image-less products into fix-it
  ...
  failing; a price rise, an unreadable price and a missing product image are all
  refused by the builder; untrusted product text is HTML-escaped into both the
